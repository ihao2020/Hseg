...
def fw_iou(intersect_area, pred_area, label_area):
    """
    Calculate fw_iou coefficient
    ...
    for i in range(len(intersect_area)):
        if union[i] == 0:
            ...
            iou = intersect_area[i] / union[i]
        w = 0 if label_area[i] == 0 \
             else label_area[i] / total_area
        fw = w * iou
        class_weight.append(w)
        class_fw.append(fw)

    fwiou = np.sum(class_fw)
    return class_weight, fwiou