...
def evaluate(model,
    ...
    class_weight, fwiou = metrics.fw_iou(*metrics_input)
    ...
    if print_detail:
        infor = "[EVAL] #Images: {} mIoU: {:.4f} fwIoU {:.4f} Acc: {:.4f} Kappa: {:.4f} Dice: {:.4f}".format(
            len(eval_dataset), miou, fwiou, acc, kappa, mdice)
        infor = infor + auc_infor if auc_roc else infor
        logger.info(infor)
        logger.info("[EVAL] Class IoU: \n" + str(np.round(class_iou, 4)))
        logger.info("[EVAL] Class Weight: \n" + str(np.round(class_weight, 4)))
        logger.info("[EVAL] Class Precision: \n" + str(
            np.round(class_precision, 4)))
        logger.info("[EVAL] Class Recall: \n" + str(np.round(class_recall, 4)))
    return miou, fwiou, acc, class_iou, class_weight, class_precision, kappa
